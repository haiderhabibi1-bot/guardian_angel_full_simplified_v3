    ...
    def _notify_admin_lawyer_registration(request, user, specialty, years_experience, bar_number):
        admin_email = settings.ADMIN_REVIEW_EMAIL
        if not admin_email:
            return
        subject = "New lawyer registration pending approval"
        body = f"""A new lawyer signed up:
...
Admin: log in and approve them to make the profile visible.
"""
    ...
    def register_lawyer(request):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                _send_verification_email(request, user)
                _notify_admin_lawyer_registration(request, user, lp.specialty, lp.years_experience, lp.bar_number)
                messages.success(request, "Lawyer account submitted. Check your email to verify your account. Admin approval required to appear publicly.")
                return redirect('login')
        else:
    ...
    def settings_lawyer(request):
        ...
        from .forms import LawyerSettingsForm
        if request.method == "POST":
            ...
            form = LawyerSettingsForm(instance=request.user, initial={
    ...
    # -------- Password reset flow (wrappers with templates) --------
